...
echo "on first setup name your wallet walletaddress and the mining pool mineaddress"
#quick test to setup config files
bin/./xmr-stak

#copy miner for systemd use
cd ~/monero_miner_deployment/xmr-stak/build/
sudo cp bin/xmr-stak /usr/local/bin/xmr-stak # so system can execute
...
echo
# Update Miner Config File With Wallet & Pool Information
sudo sed -i "s/walletaddress/$WALLET/" /usr/local/bin/xmr-stak/config.txt
sudo sed -i "s/mineaddress/$MINEHOST/" /usr/local/bin/xmr-stak/pool.txt
